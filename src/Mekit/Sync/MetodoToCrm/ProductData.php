...
class ProductData extends Sync implements SyncInterface
{
      ...
      if (isset($mainCat->crm_id))
      {
        ...
        foreach ($mainCat->children as $childCatId => &$childCat)
        {
          try
          {
            ...
            {
              $this->log(
                "PRODUCT CHILD CATEGORY('" . $childCat->name . "') ID: " . $childCat->crm_id . " MAIN CATEGORY ID: "
                . $mainCatCrmId
              );
            }
  ...
  protected function saveProductInCache($localItem)
  {
    ...
    //$this->log("METODO-UD: " . $metodoLastUpdateTime->format('c'));
    //$this->log("CACHED-UD: " . $updateItemLastUpdateTime->format('c'));

    if (intval($metodoLastUpdateTime->format('U')) > intval($updateItemLastUpdateTime->format('U')))
    {
  ...
  protected function getNextProduct($database)
  {
    if (!$this->localItemStatement)
    {
      ...
      $sql = "SELECT
                ...
                P.LA42_DATAMODIFICA,
                P.LA9997_DATAMODIFICA,
                P.LA9998_DATAMODIFICA,
    ...
    if ($item)
    {
      ...
      $lastModDate = $this->getLatestModificationDate($item);
      $item->metodo_last_update_time_c = $lastModDate->format("Y-m-d H:i:s.u");
  ...
  /**
   * there is one on the article and one (x5) on each listino - will use the latest
   *
   * @param \stdClass $item
   * @return \DateTime
   */
  protected function getLatestModificationDate($item)
  {
    $dates = [];

    if ($item->metodo_last_update_time_c)
    {
      $dates[] = \DateTime::createFromFormat('Y-m-d H:i:s.u', $item->metodo_last_update_time_c);
    }

    if ($item->LA42_DATAMODIFICA)
    {
      $dates[] = \DateTime::createFromFormat('Y-m-d H:i:s.u', $item->LA42_DATAMODIFICA);
    }

    if ($item->LA9997_DATAMODIFICA)
    {
      $dates[] = \DateTime::createFromFormat('Y-m-d H:i:s.u', $item->LA9997_DATAMODIFICA);
    }

    if ($item->LA9998_DATAMODIFICA)
    {
      $dates[] = \DateTime::createFromFormat('Y-m-d H:i:s.u', $item->LA9998_DATAMODIFICA);
    }

    if ($item->LA9999_DATAMODIFICA)
    {
      $dates[] = \DateTime::createFromFormat('Y-m-d H:i:s.u', $item->LA9999_DATAMODIFICA);
    }

    if ($item->LA10000_DATAMODIFICA)
    {
      $dates[] = \DateTime::createFromFormat('Y-m-d H:i:s.u', $item->LA10000_DATAMODIFICA);
    }
